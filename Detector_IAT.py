...
def get_IAT_features(path,hasher):
	...
	if hasattr(pe,"DIRECTORY_ENTRY_IMPORT") != False:
    	for imp in pe.DIRECTORY_ENTRY_IMPORT:
            if imp.imports[0].name:
				now = imp.imports[0].name.decode()
				print(now)
                import_func_features[now] = 1

    # extract strings from binary file using regular expressions
    # hash the features using the hashing trick
    hashed_features = hasher.transform([import_func_features])
    ...
    print("Extracted {0} function names from {1}".format(len(import_func_features),path))
